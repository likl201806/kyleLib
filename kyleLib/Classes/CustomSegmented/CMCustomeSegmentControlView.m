...
@interface CMCustomeSegmentControlView()
...
@property (nonatomic, strong) UIImageView * heightColoreView;
...
@implementation CMCustomeSegmentControlView
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)customeData {
    ...
    if (_backgroundHeightLightStartColor == nil){
        _backgroundHeightLightStartColor = [UIColor redColor];
    }
    
    if (_backgroundHeightLightEndColor == nil){
        _backgroundHeightLightEndColor = [UIColor redColor];
    }
...
- (void) createTopLables {
    ...
    _heightColoreView = [[UIImageView alloc] initWithFrame:heightLightViewFrame];
    _heightColoreView.backgroundColor = [UIColor clearColor];
    /**
     采用方法创建渐变Image
     */
    UIImage *gradualImg = [UIImage imageNamed:@"sdf_create_wifi_bg"];
    _heightColoreView.image = gradualImg;
    _heightColoreView.contentMode = UIViewContentModeScaleAspectFit;
    _heightColoreView.layer.cornerRadius = 0.5*heightLightViewFrame.size.height;
    _heightColoreView.layer.masksToBounds = YES;
