...
#define ly_accountKey @"ly_accountKey"
typedef enum {
...
@implementation LYDataStoreCache
#pragma mark - NSUserDefaults
#pragma mark 将数据(对象)写入用户偏好
+(void)saveObj:(id)obj forKey:(NSString *)key{
    if(![LYSaveDataType isFoudationClass:obj]){
        obj= [obj ly_toDic];
    }
    if([obj isKindOfClass:[NSArray class]]){
        ...
        for (id subObj in obj) {
            if(![LYSaveDataType isFoudationClass:subObj]){
                [resArr addObject:[subObj ly_toDic]];
            }else{
...
+(void)arcObj:(id)obj pathComponent:(NSString *)pathComponent{
    NSString *absPath = [LYDocPath stringByAppendingPathComponent:pathComponent];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if(!obj){
        PathAttr pathAttr = [self getPathAttrWithPathComponent:pathComponent];
        if(pathAttr == PathAttrNotExist){
            LYDataHandleLog(@"删除失败！路径:%@不存在！",pathComponent);
        }else{
            NSError *error;
            BOOL res = [fileManager removeItemAtPath:absPath error:&error];
            if(!res){
                LYDataHandleLog(@"删除失败！路径:%@;原因:%@",pathComponent,error.description);
            }
        }
        return;
    }
    if(![LYSaveDataType isFoudationClass:obj] && obj){
        if(!([obj respondsToSelector:@selector(encodeWithCoder:)] && [obj respondsToSelector:@selector(initWithCoder:)])){
            LYDataHandleLog(@"归档失败！对象%@未实现encodeWithCoder:或initWithCoder:",obj);
            return;
        }
    }
    if([pathComponent containsString:@"/"]){
        ...
        if(pathAttr == PathAttrNotExist){
            [self creatDirWithPathComponent:[LYDocPath stringByAppendingPathComponent:dirPath]];
        }
    }else if([self getPathAttrWithPathComponent:pathComponent] == PathAttrDir){
        LYDataHandleLog(@"归档失败！路径:%@已存在同名文件夹！",pathComponent);
        return;
    }
    BOOL res = [NSKeyedArchiver archiveRootObject:obj toFile:absPath];
    if(!res){
        LYDataHandleLog(@"归档失败！路径:%@",pathComponent);
    }
}
...
+(id)unArcObjPathComponent:(NSString *)pathComponent{
    PathAttr pathAttr = [self getPathAttrWithPathComponent:pathComponent];
    if(pathAttr == PathAttrNotExist){
        LYDataHandleLog(@"数据读取失败！路径:%@不存在！",pathComponent);
        return nil;
    }
    return [NSKeyedUnarchiver unarchiveObjectWithFile:[LYDocPath stringByAppendingPathComponent:pathComponent]];
}
#pragma mark - User
...
+(void)saveUserAccount:(NSString *)userAccount{
    [self saveObj:userAccount forKey:ly_accountKey];
}
#pragma mark 读取用户数据存储的account
+(NSString *)readUserAccount{
    return [self readObjForKey:ly_accountKey];
}
...
+(void)saveUserObj:(id)obj pathComponent:(NSString *)pathComponent{
    NSString *accountKey = [self readObjForKey:ly_accountKey];
    if(accountKey){
...
+(void)cleanCacheData{
    NSArray *subPathArr = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:LYDocPath error:nil];
    NSString *filePath;
    NSError *error;
    for (NSString *subPath in subPathArr){
        filePath = [LYDocPath stringByAppendingPathComponent:subPath];
        [[NSFileManager defaultManager] removeItemAtPath:filePath error:&error];
        if (error) {
            LYDataHandleLog(@"清除失败！路径:%@",filePath);
        }
    }
}
...
+(void)creatDirWithPathComponent:(NSString *)pathComponent{
    ...
    if(pathAttr == PathAttrNotExist){
        ...
        if(!res){
            LYDataHandleLog(@"归档失败！文件夹创建失败！路径:%@;原因:%@",pathComponent,error.description);
        }
    }
}
...
+(PathAttr)getPathAttrWithPathComponent:(NSString *)pathComponent{
    BOOL isDir = NO;
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL pathExist = [fileManager fileExistsAtPath:[LYDocPath stringByAppendingPathComponent:pathComponent] isDirectory:&isDir];
    if(!pathExist){
