...
@implementation NSObject (LYSQliteHandle)

#pragma mark 存储数据到当前表中（支持批量存储）
-(BOOL)ly_dbSave{
    if([LYSaveDataType isFoudationClass:self]){
        if([LYSaveDataType ly_dataType:self] == DataTypeArr){
            if([self isKindOfClass:[NSArray class]]){
                ...
                for (id subObj in objArr) {
                    if([subObj isKindOfClass:[NSArray class]]){
                        ...
                        if(![LYSaveDataType isFoudationClass:subObj]){
                            BOOL creatTableSucc = [[subObj class] ly_dbCreatTable];
                            if(creatTableSucc){
                                BOOL updateTableSucc = [[subObj class] ly_dbUpdateTableFields];
                                if(updateTableSucc){
                                    NSString *insertSql = [self getAlertSqlWithObj:subObj alertType:AlertTypeInsert];
                                    [sqlArr addObject:insertSql];
                                }else{
                                    LYDataHandleLog(@"表字段更新出错，请检查表：%@",NSStringFromClass([subObj class]));
                                }
                            }else{
                                 LYDataHandleLog(@"表创建失败，表名：%@",NSStringFromClass([subObj class]));
                                return NO;
                            }
                        }
                ...
                LYSQLResult *res = [LYDataStoreSQlite executeSqls:sqlArr];
                return res.success;
            }
        }
        return NO;
    }
    BOOL creatTableSucc = [[self class] ly_dbCreatTable];
    if(creatTableSucc){
        BOOL updateTableSucc = [[self class] ly_dbUpdateTableFields];
        if(updateTableSucc){
            NSString* insertSql = [self getAlertSqlWithObj:self alertType:AlertTypeInsert];
            if(insertSql.length){
                LYSQLResult *res = [LYDataStoreSQlite executeSql:insertSql res:NO];
                return res.success;
            }
        }else{
            LYDataHandleLog(@"表字段更新出错，请检查表：%@",NSStringFromClass([self class]));
        }
        
        return NO;
    }else{
        LYDataHandleLog(@"表创建失败，表名：%@",NSStringFromClass([self class]));
    }
    return NO;
    
}

#pragma mark 从当前表中删除数据，where语句支持字典或sql字符串，字典默认使用AND连接，键值对比较为等号，其他情况请使用ly_dbDropWhereArg
+(BOOL)ly_dbDropWhere:(id)where{
    ...
    LYSQLResult *res = [LYDataStoreSQlite executeSql:dropSql res:NO];
    return res.success;
}
...
+(BOOL)ly_dbDropWhereArg:(NSString *)arg1,...{
    va_list args;
    va_start(args,arg1);
    return [[self ly_dbAlertWhereArg:@selector(ly_dbDropWhere:)returnType:ReturnTypeBool owner:self args:args arg1:arg1] boolValue];
}

#pragma mark 从当前表中查询数据并直接返回对象数组，where语句支持字典或sql字符串，字典默认使用AND连接，键值对比较为等号，其他情况请使用ly_dbQuaryWhereArg
+(NSArray *)ly_dbQuaryWhere:(id)where{
    ...
    LYSQLResult *res = [LYDataStoreSQlite executeSql:quarySql res:YES];
    if(res.resData.count){
        NSArray *selfModelArr = [self ly_modelWithObj:res.resData];
        return selfModelArr;
    }
    return nil;
}
...
+(NSArray *)ly_dbQuaryWhereArg:(NSString *)arg1,...{
    va_list args;
    va_start(args,arg1);
    return [self ly_dbAlertWhereArg:@selector(ly_dbQuaryWhere:)returnType:ReturnTypeArr owner:self args:args arg1:arg1];
}
#pragma mark 从当前表中查询所有数据
+(NSArray *)ly_dbQuaryAll{
    return [self ly_dbQuaryWhere:@""];
}

#pragma mark 根据当前对象从当前表中修改数据，where语句支持字典或sql字符串，字典默认使用AND连接，键值对比较为等号，其他情况请使用ly_dbUpdateWhereArg
-(BOOL)ly_dbUpdateWhere:(id)where{
    ...
    if(updateTableSucc){
        ...
        LYSQLResult *res = [LYDataStoreSQlite executeSql:updateSql res:NO];
        return res.success;
    }else{
        LYDataHandleLog(@"表字段更新出错，请检查表：%@",TbName);
        return NO;
    }
...
-(BOOL)ly_dbUpdateWhereArg:(NSString *)arg1,...{
    va_list args;
    va_start(args,arg1);
    return [[[self class] ly_dbAlertWhereArg:@selector(ly_dbUpdateWhere:)returnType:ReturnTypeBool owner:self args:args arg1:arg1] boolValue];
}
#pragma mark 传入一个字典从当前表中修改数据，where语句支持字典或sql字符串，字典默认使用AND连接，键值对比较为等号，其他情况请使用ly_dbUpdateDic:whereArg:
+(BOOL)ly_dbUpdateDic:(NSDictionary *)updateDic where:(id)where{
    ...
    [self getEnumPropertyNamesCallBack:^(NSString *proName, NSString *proType) {
        id valObj = [updateDic ly_dicSafetyReadForKey:proName];
        if(valObj){
            int numValue = -1;
            DataType dataType = [LYSaveDataType ly_dataType:valObj];
...
+(BOOL)ly_dbUpdateDic:(NSDictionary *)updateDic whereArg:(NSString *)arg1,...{
    ...
    return [self ly_dbUpdateDic:updateDic where:whereSql];
}

#pragma mark 创建当前表
+(BOOL)ly_dbCreatTable{
    if([self ly_dbExistTableWithTbName:TbName]){
        return YES;
    }
    ...
    [self getEnumPropertyNamesCallBack:^(NSString *proName, NSString *proType) {
        NSString *sqlValueType = [LYDataStoreSQlite getSqlValueTypeWithProType:proType];
    ...
    if(hasValue){
        ...
        LYSQLResult *res = [LYDataStoreSQlite executeSql:creatTableSql res:NO];
        return res.success;
    }
    return NO;
}
...
+(BOOL)ly_dbUpdateTableFields{
    if([[LYDataStoreSQlite shareInstance].allJudgedUpdateTb containsObject:TbName]){
        return YES;
    }
    NSString *tbNamesSql = [NSString stringWithFormat:@"PRAGMA TABLE_INFO(%@)",TbName];
    LYSQLResult *res = [LYDataStoreSQlite executeSql:tbNamesSql res:YES];
    NSMutableArray *fieldsArr = [NSMutableArray array];
    NSMutableDictionary *prosMuDic = [NSMutableDictionary dictionary];
    [self getEnumPropertyNamesCallBack:^(NSString *proName, NSString *proType) {
        NSString *sqlValueType = [LYDataStoreSQlite getSqlValueTypeWithProType:proType];
        if(sqlValueType.length){
            [prosMuDic ly_dicSaftySetValue:sqlValueType forKey:proName];
        }
    }];
    for (NSDictionary *resDic in res.resData) {
        NSString *fieldStr = [resDic ly_dicSafetyReadForKey:@"name"];
    ...
    for (NSString *name in proKeysArr) {
        NSString *proType = [prosMuDic ly_dicSafetyReadForKey:name];
        if(proType.length){
            NSString *preSql = [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN ",TbName];
            preSql = [preSql stringByAppendingString:[NSString stringWithFormat:@"%@ %@",name,[prosMuDic ly_dicSafetyReadForKey:name]]];
            BOOL succ = [LYDataStoreSQlite executeSql:preSql res:NO].success;
            if(!succ){
                allSucc = NO;
            }
        }
    ...
    if(allSucc){
        [[LYDataStoreSQlite shareInstance].allJudgedUpdateTb ly_arrSafetyAddObjNORepetition:TbName];
    }
    return allSucc;
}
#pragma mark 删除当前表
+(BOOL)ly_dbDropTable{
    ...
    LYSQLResult *res = [LYDataStoreSQlite executeSql:dropTableSql res:NO];
    if(res.success){
        [[LYDataStoreSQlite shareInstance].allJudgedExistTb ly_arrSafetyRemoveObj:TbName];
    }
    return res.success;
}
#pragma mark - Private
#pragma mark 判断当前表是否存在
+(BOOL)ly_dbExistTableWithTbName:(NSString *)tbName{
    if([[LYDataStoreSQlite shareInstance].allJudgedExistTb containsObject:tbName]){
        return YES;
    }
    NSString *isExistsSql = [NSString stringWithFormat:@"SELECT COUNT(*) FROM SQLITE_MASTER WHERE TYPE='table' AND NAME='%@'",tbName];
    LYSQLResult *res1 = [LYDataStoreSQlite executeSql:isExistsSql res:YES];
    NSArray *countsArr = res1.resData;
    if(countsArr && countsArr.count){
        NSDictionary *firstDic = countsArr.firstObject;
        int count = [[firstDic ly_dicSafetyReadForKey:@"COUNT(*)"] intValue];
        if(count != 0){
            [[LYDataStoreSQlite shareInstance].allJudgedExistTb ly_arrSafetyAddObjNORepetition:tbName];
            return YES;
        }
    }
    return NO;
...
+(id)ly_dbAlertWhereArg:(SEL)sel returnType:(ReturnType)returnType owner:(id)owner args:(va_list)args arg1:(NSString *)arg1,... {
    NSString *whereSql = @"";
    id eachObject;
    if (arg1) {
        whereSql = [whereSql stringByAppendingString:[NSString stringWithFormat:@"%@ ",arg1]];
...
+(NSString *)getWhereSqlWithObj:(id)obj{
    DataType dataType = [LYSaveDataType ly_dataType:obj];
    if(dataType == DataTypeDic){
        __block NSString *whereSql = @"";
        [obj enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
            NSString *symbol = @"=";
            for (NSString *supSymbol in [LYDataStoreSQlite sqlSymbolArr]) {
...
-(NSString *)getAlertSqlWithObj:(id)obj alertType:(AlertType)alertType{
    ...
    for (NSString *proName in proNamesArr) {
        id value = [obj ly_objSafetyReadForKey:proName];
        int legalValue = -1;
