...
@implementation CMDFixedTabbarViewTabItem
@end

@implementation CMDFixedTabbarView {
...
- (void)setTabbarItems:(NSArray *)tabbarItems{
    if (_tabbarItems != tabbarItems) {
        ...
        float x = 0.0f;
        NSInteger i=0;
        for (CMDFixedTabbarViewTabItem *item in tabbarItems) {
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(x, 0, width, height)];
            label.text = item.title;
            label.font = kCMDFont(15.0);
...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    CMDFixedTabbarViewTabItem *fromItem = [self.tabbarItems objectAtIndex:fromIndex];
    ...
    fromLabel.textColor = [CMDUtility getColorOfPercent:percent between:fromItem.titleColor and:fromItem.selectedTitleColor];
    fromIamge.alpha = percent;
    fromSelectedIamge.alpha = (1-percent);

    if (toIndex >= 0 && toIndex < [self tabbarCount]) {
        CMDFixedTabbarViewTabItem *toItem = [self.tabbarItems objectAtIndex:toIndex];
        ...
        toLabel.textColor = [CMDUtility getColorOfPercent:percent between:toItem.selectedTitleColor and:toItem.titleColor];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    if (_selectedIndex != selectedIndex) {
        if (_selectedIndex >= 0) {
            CMDFixedTabbarViewTabItem *fromItem = [self.tabbarItems objectAtIndex:_selectedIndex];
        ...
        if (selectedIndex >= 0 && selectedIndex < [self tabbarCount]) {
            CMDFixedTabbarViewTabItem *toItem = [self.tabbarItems objectAtIndex:selectedIndex];
